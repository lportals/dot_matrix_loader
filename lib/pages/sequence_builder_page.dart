...
class _SequenceBuilderPageState extends State<SequenceBuilderPage> {
  ...
  final Color _inactiveColor = const Color(0xFF1C1C1C);
  ...
  void _onGridSizeChange(int newRows, int newCols) async {
    // If we have meaningful data, ask for confirmation
    if (_frames.length > 1 || _frames[0].any((r) => r.any((c) => c))) {
      final confirm = await showDialog<bool>(
        ...
        builder: (ctx) => AlertDialog(
          ...
          content: Text(
            'Changing the grid size will delete all current frames. Are you sure?',
    ...
    setState(() {
      _rows = newRows;
      _cols = newCols;
      _frames = [_createEmptyFrame(_rows, _cols)];
      _currentIndex = 0;
    });
  }
  ...
  Widget build(BuildContext context) {
    return InkWell(
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            Icon(icon, color: onTap == null ? color.withOpacity(0.3) : color),
